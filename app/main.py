...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    # Support in-memory session tokens alongside JWT
    if "_session_store" in globals():
        session = _session_store.get(token)
        if session:
            role = session.get("role", "hospital")
            if role == "hospital":
                role = "admin"
            return {"username": session.get("user_id"), "role": role}
    try:
...
class RequireRole:
    ...
    def __call__(self, user: dict = Depends(get_current_user)):
        ...
        if user_role not in self.allowed_roles and not (user_role == "hospital" and "admin" in self.allowed_roles):
...
_SEED_DOCTORS: list = [
    ...
    },
    {
        ...
        "license_no": "MED-11001-DL",
        "state": "Delhi",
        "email": "asharma@cityhospital.in",
        "hospital_name": "City General Hospital",
        "phone": "+91-9811001100",
        "password": "doctor",
    }
...
def _init_persist_db():
    ...
    with _db() as conn:
        ...
        # Handle migrations for existing tables safely
        for alter_sql in [
            "ALTER TABLE doctors ADD COLUMN IF NOT EXISTS password TEXT",
            "ALTER TABLE hospitals ADD COLUMN IF NOT EXISTS password TEXT",
            "ALTER TABLE patients ADD COLUMN IF NOT EXISTS has_imaging INTEGER DEFAULT 0",
            "ALTER TABLE patients ADD COLUMN IF NOT EXISTS has_lab INTEGER DEFAULT 0",
            "ALTER TABLE patients ADD COLUMN IF NOT EXISTS has_notes INTEGER DEFAULT 0",
            "ALTER TABLE patients ADD COLUMN IF NOT EXISTS reports TEXT DEFAULT '{}'",
            "ALTER TABLE patients ADD COLUMN IF NOT EXISTS reports_content TEXT DEFAULT '{}'",
        ]:
            try:
                with conn.cursor() as c:
                    c.execute(alter_sql)
            ...
            for p in MOCK_PATIENTS:
                c.execute("""INSERT INTO patients 
                    ...
                    ON CONFLICT (id) DO UPDATE SET 
                        name=EXCLUDED.name, age=EXCLUDED.age, symptoms=EXCLUDED.symptoms,
                        has_imaging=EXCLUDED.has_imaging, has_lab=EXCLUDED.has_lab, has_notes=EXCLUDED.has_notes,
                        reports=EXCLUDED.reports, reports_content=EXCLUDED.reports_content""",
            ...
            for d in _SEED_DOCTORS:
                c.execute("""INSERT INTO doctors 
                    ...
                    ON CONFLICT (license_no) DO UPDATE SET 
                        full_name=EXCLUDED.full_name, state=EXCLUDED.state, email=EXCLUDED.email,
                        hospital_name=EXCLUDED.hospital_name, phone=EXCLUDED.phone, password=EXCLUDED.password""",
            ...
            for h in _SEED_HOSPITALS:
                c.execute("""INSERT INTO hospitals 
                    ...
                    ON CONFLICT (reg_no) DO UPDATE SET 
                        hospital_name=EXCLUDED.hospital_name, address=EXCLUDED.address,
                        admin_name=EXCLUDED.admin_name, admin_email=EXCLUDED.admin_email,
                        email=EXCLUDED.email, phone=EXCLUDED.phone, password=EXCLUDED.password, role=EXCLUDED.role""",
