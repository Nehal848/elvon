...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      token = session.token || ""
      role = session.role || "hospital"
    } catch(e) {}

    const headers: any = {}
    if (token) headers["Authorization"] = `Bearer ${token}`

    const endpoint = role === "hospital" ? "/api/hospital/dashboard" : "/api/doctor/dashboard"
      ...
      .then(data => {
        if (data) {
          setDashData((prev: any) => ({
            ...
            active_models: data.active_models || prev.active_models || [],
          }))
        }
      })
      .catch(err => {
        console.warn("Dashboard using offline enclave mock cache:", err)
      })
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="bg-white rounded-3xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-slate-100 flex flex-col relative overflow-hidden h-[160px]">
                <div className="flex items-start gap-4 mb-2">
                  ...
                  <div>
                    <div className="text-[22px] font-semibold text-slate-900 leading-tight">{dashData?.alerts?.length ?? 5}</div>
              ...
              <div className="bg-white rounded-3xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-slate-100 flex flex-col justify-between h-[160px]">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="text-[22px] font-semibold text-slate-900 leading-tight">{dashData?.active_models?.length ?? 3}</div>
